...
# Try networkD3 first (more compatible), then visNetwork
interactive_created <- FALSE

if (requireNamespace("networkD3", quietly = TRUE)) {
  cat("Creating Visualization 4: Interactive HTML version (networkD3)...\n")
  
  library(networkD3)
  
  # Prepare data for networkD3 (needs 0-indexed nodes)
  edge_list <- as_edgelist(conflict_graph, names = FALSE) - 1  # 0-indexed
  
  # Create node data frame
  nodes_d3 <- data.frame(
    name = V(conflict_graph)$label,
    group = greedy_colors,
    size = V(conflict_graph)$degree
  )
  ...
  # Save to HTML
  library(htmlwidgets)
  saveWidget(network_d3, "conflict_graph_interactive.html", selfcontained = TRUE)
  
  cat("  Saved: conflict_graph_interactive.html\n")
  cat("  (Open in web browser - drag nodes, zoom, hover for details)\n\n")
  interactive_created <- TRUE
  
} else if (requireNamespace("visNetwork", quietly = TRUE)) {
  cat("Creating Visualization 4: Interactive HTML version (visNetwork)...\n")
...
if (!interactive_created) {
  cat("  Skipping interactive visualization\n")
  cat("  To enable, install one of:\n")
  cat("    install.packages('networkD3')  # Recommended for older R\n")
  cat("    install.packages('visNetwork') # Requires R >= 4.0\n\n")
}
